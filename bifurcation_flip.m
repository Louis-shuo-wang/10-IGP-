% plot_flip_by_gamma.m
% Demonstrate flip (period-doubling) when gamma crosses a critical value
% using the forward-Euler map with the parameters found earlier.

clear; close all; clc;

%% --- Parameters from your search (flip case) ---
alpha=2.304;
beta=1.909;
gamma0=0.2067;
delta=0.4913;
...
% alpha   = 1.516;
% beta    = 0.6098;
% gamma0  = 0.04542;   % reported critical gamma from your search
% delta   = 0.4864;
% epsilon = 1.544;
% dt_flip = 1.341;     % Δτ that produced the flip in the search

%% --- Choose gammas below and above the critical value ---
dg = 0.01;                       % small offset around gamma0; tweak if needed
...
steps = 1600;    % total iterations (transient + sample)
transient = 800; % drop first 'transient' iterates when plotting

traj_below = iterate_euler(dt_flip, steps, X0_b, alpha, beta, delta, epsilon, gamma_below);
traj_above = iterate_euler(dt_flip, steps, X0_a, alpha, beta, delta, epsilon, gamma_above);
...
% (a) P_n below
nexttile;
plot(t(transient+1:5:end), traj_below(transient+1:5:end,2));
xlabel('n'); ylabel('$P_n$'); 
...
% (b) P_n above
nexttile;
plot(t(transient+1:5:end), traj_above(transient+1:5:end,2));
xlabel('n'); ylabel('$P_n$'); 
...
% sgtitle('Flip bifurcation demonstration (varying \gamma, fixed \Delta\tau)');

% save figure if desired
% exportgraphics(gcf,'flip_by_gamma.svg','Resolution',600);

%% --- quick diagnostics printed to command window ---
mu_b = eig(eye(3) + dt_flip * J_b);
mu_a = eig(eye(3) + dt_flip * J_a);
...
drawnow;
exportgraphics(gcf,'Fig6.svg','Resolution',600);

% plot_fold_by_gamma.m
% Demonstrate a fold (saddle-node) bifurcation for the forward-Euler map.
% Uses the fold parameter set discovered in your search:
%   alpha=0.2646, beta=1.453, gamma_crit≈1.151, delta=0.239, epsilon=1.939, dt=0.0001

clear; close all; clc;

% --- Parameters (fold case) ---
...
% alpha   = 0.2646;
% beta    = 1.453;
% gamma0  = 1.151;    % reported critical gamma (fold)
% delta   = 0.239;
% epsilon = 1.939;
% dt_fold = 0.0001;   % Δτ that produced the fold in your search
...
traj_below = iterate_euler(dt_fold, steps, X0_b, alpha, beta, delta, epsilon, gamma_below);
traj_above = iterate_euler(dt_fold, steps, X0_a, alpha, beta, delta, epsilon, gamma_above);
...
mu_b = eig(eye(3) + dt_fold * J_b);
mu_a = eig(eye(3) + dt_fold * J_a);
...
drawnow;
exportgraphics(gcf,'Fig5.svg','Resolution',600);

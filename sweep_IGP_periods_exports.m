%% Leslie–Gower IGP: 1D gamma sweep + 2D scans
%% with limit-cycle confirmation, period estimation, and CSV/MAT exports
%   dx/dt = x(1 - x/z) - x y
...
% 5: CONFIRMED limit cycle by integration  (period estimated)
...
% boundary P2 (predator-only): (0, gamma/epsilon, beta*gamma/epsilon)
yP2 = G./epsilon; zP2 = beta*yP2; xP2 = zeros(size(G));
...
defaultsetting;
f1 = figure('Name','1D sweep: x* vs gamma','Color','w', 'Position', [100,100,900,450]); hold on; grid on; box on;
plot_branch(G, x1, stab1, [0 0.45 0.75], 1.8);
plot_branch(G, x2, stab2, [0.85 0.33 0.10], 1.8);
plot(G, zeros(size(G)), ':', 'Color', [0 0 0 0.25]); % x=0 baseline
xline(gamma_SN, '--k', 'Label','$\gamma_{SN}$', 'LabelVerticalAlignment','middle');
xline(epsilon, ':k', 'Label','$\epsilon$', 'LabelVerticalAlignment','middle');
...
% rough Hopf markers (largest real part ~ 0) on branch 1
[gh1, ~] = hopf_markers(G, x1, y1, z1, alpha, beta, delta, epsilon);
if ~isempty(gh1), xline(gh1, '-', 'Color',[0.2 0.6 0.2], 'Label','Hopf (br.1)'); end

%% ---------------- 2) Two-parameter map (gamma, epsilon) ----------------
Ng2 = 200; Ne2 = 200;               % tune for accuracy vs runtime
G2 = linspace(0.01, 1.2, Ng2);
E2 = linspace(0.02, 0.8,  Ne2);

confirmOsc = true;                % integration-based confirmation
mapTmax    = 400;                 % integration horizon
mapTrans   = 0.5;                 % transient fraction to discard
ampThresh  = 1e-3;                % amplitude threshold for cycle
IC         = [0.15; 0.12; 0.18];  % default IC for confirmation

classGE  = zeros(Ne2, Ng2);
periodGE = nan(Ne2, Ng2);         % store estimated period where class==5
...
for ie = 1:Ne2
    ...
    for ig = 1:Ng2
        ...
        if confirmOsc && classGE(ie, ig) == 3
            ...
            [isLC, ~, Tper] = confirm_limit_cycle(p, IC, mapTmax, mapTrans, ampThresh);
            if isLC
                classGE(ie, ig)  = 5;         % upgrade to confirmed limit cycle
                periodGE(ie, ig) = Tper;      % store period (NaN if not resolved)
            end
        end
    end
end
...
title('$(\gamma , \epsilon)$ regime map');
...
title('$(\gamma , \beta)$ regime map');
cb3 = colorbar; cb3.Label.String = 'class';
colormap(class_cmap());
set(gca,'CLim',[-0.5 5.5]);
set_colorbar_labels(cb3);

%% ---------------- Period contours overlaid on 2D maps ----------------
% Overlay on (gamma, epsilon)
overlay_period_contours(f2.CurrentAxes, G2, E2, periodGE, classGE, 6);
% re-export with contours
outdir = 'figs'; if ~exist(outdir,'dir'), mkdir(outdir); end
save_vec(f2, fullfile(outdir,'map_gamma_epsilon_with_period_contours.pdf'));

% Overlay on (gamma, beta)
overlay_period_contours(f3.CurrentAxes, G3, B3, periodGB, classGB, 6);
save_vec(f3, fullfile(outdir,'map_gamma_beta_with_period_contours.pdf'));
...
%% ---------------- 5) CSV/MAT exports ----------------
datadir = 'exports'; if ~exist(datadir,'dir'), mkdir(datadir); end

% 1D sweep export
T1 = table(G(:), x1(:), y1(:), z1(:), stab1(:), x2(:), y2(:), z2(:), stab2(:), ...
    'VariableNames', {'gamma','x1','y1','z1','stab1','x2','y2','z2','stab2'});
writetable(T1, fullfile(datadir,'gamma_sweep_branches.csv'));

% 2D (gamma,epsilon) map exports
writematrix(G2, fullfile(datadir,'map_gamma_epsilon_gamma_axis.csv'));
writematrix(E2, fullfile(datadir,'map_gamma_epsilon_epsilon_axis.csv'));
writematrix(classGE, fullfile(datadir,'map_gamma_epsilon_classes.csv'));
writematrix(periodGE, fullfile(datadir,'map_gamma_epsilon_periods.csv')); % NaN where not class 5

% 2D (gamma,beta) map exports
writematrix(G3, fullfile(datadir,'map_gamma_beta_gamma_axis.csv'));
writematrix(B3, fullfile(datadir,'map_gamma_beta_beta_axis.csv'));
writematrix(classGB, fullfile(datadir,'map_gamma_beta_classes.csv'));
writematrix(periodGB, fullfile(datadir,'map_gamma_beta_periods.csv'));    % NaN where not class 5

% Bundle MAT
save(fullfile(datadir,'regime_maps.mat'), ...
    'alpha','beta','delta','epsilon', ...
     'G','x1','y1','z1','stab1','x2','y2','z2','stab2', ...
     'G2','E2','classGE','periodGE', ...
     'G3','B3','classGB','periodGB', ...
     'gamma_SN','gh1');

fprintf('Saved vectors to: %s\nSaved data to: %s\n', outdir, datadir);
...
function [isLC, amp, Tperiod] = confirm_limit_cycle(p, x0, T, fracTransient, ampThresh)
    % Integrate and confirm sustained oscillations; estimate period (robust)
    ...
    idx  = find(t >= tcut);
    if numel(idx) < 12
        isLC = false; amp = 0; Tperiod = NaN; return;
    end
    ...
    isLC = amp > ampThresh;
    % estimate period using the variable with largest amplitude
    [~, whichVar] = max([ampX ampY ampZ]);
    switch whichVar
        case 1, sig = Xs;
        case 2, sig = Ys;
        otherwise, sig = Zs;
    ...
    if isLC
        Tperiod = estimate_period(t(idx), sig);
    else
        Tperiod = NaN;
    end
end
...
function Tper = estimate_period(t, s)
    % Robust period estimate via peaks, then autocorr, then FFT fallback
    s = s - mean(s);
    n = numel(s);
    ...
    ac = xcorr(s, 'coeff');
    mid = (numel(ac)+1)/2;
    acp = ac(mid+1:end);                   % positive lags
    % look for first significant local max after zero
    L = numel(acp);
    if L >= 5
        jj = 2:L-1;
...
function overlay_period_contours(ax, xgrid, ygrid, Pperiod, classMap, nLevels)
% Draws period contours on top of an existing imagesc map.
%   - Only plots where classMap == 5 (confirmed limit cycle).
%   - nLevels: number of contour levels to draw.
    if nargin < 6, nLevels = 6; end
    Pmask = Pperiod;
    Pmask(classMap ~= 5) = NaN;        % mask everything but confirmed cycles
    finiteVals = Pmask(~isnan(Pmask) & isfinite(Pmask));
    if numel(finiteVals) < 3
        return; % nothing to contour
    end
    % Build grids for contour (imagesc already set axes; contours align)
    [X, Y] = meshgrid(xgrid, ygrid);
    % Choose nice contour levels (log-spacing if wide dynamic range)
    levels = choose_levels(finiteVals, nLevels);
    axes(ax); hold(ax,'on');
    [C,h] = contour(ax, X, Y, Pmask, levels, 'LineColor','k', 'LineWidth',1.1);
    try
        clabel(C, h, 'Color','k', 'FontSize',9, 'LabelSpacing',250);
    catch
        % clabel can fail if contour is empty; ignore
    end
    % Small annotation to clarify overlay
    text(ax, 0.99, 0.02, 'Period contours (class 5)',...
        'Units','normalized','HorizontalAlignment','right', ...
        'VerticalAlignment','bottom','Color','k','FontSize',9, ...
        'BackgroundColor',[1 1 1 0.5],'Margin',2);
end

function levels = choose_levels(vals, n)
% Compute "nice" contour levels from finite period values.
    ...
    spread = vmax / vmin;
    if spread > 5   % wide dynamic range -> log levels
        levels = logspace(log10(vmin), log10(vmax), n);
    else            % modest spread -> linear levels
        levels = linspace(vmin, vmax, n);
    end
end